...
for dirpath, dirnames, filenames in os.walk("Bentley"): #is there something quicker than os.walk?
    for filename in [f for f in filenames if f.endswith("GILL.MAG")]:
        all_GILL_files.append(os.path.join(dirpath,filename))
        
print(all_GILL_files)
findfile_time = time.time()
print('Time to find GILL files '+str(findfile_time - start_time)+'s')
...
max_mth_entries = (31+1)*24*60*60/5
for year in years:
    print('Doing GILL files for year ',year)
