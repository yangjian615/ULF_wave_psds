...
%% Test functions
% 1 Failure for bad gen_opts.speed_sector input
% 2 Failure for bad gen_opts.speed_sector input
% 3 Failure for bad gen_opts.speed_sector input
% 4 Failure for bad gen_opts.speed_sector input
% 5 Failure for bad gen_opts.speed_sector input
% 6 Pass good gen_opts.speed_sector input
% 7 Failure for bad gen_opts.speed_sector input


function test_check_basic_struct_1(testCase)
% Test 1: throw out non-cell inputs
	gopts = make_basic_struct('gen_opts');
	gopts.speed_sectors = [1:6];
	

	verifyError(testCase,@()check_basic_struct(gopts,'gen_opts'),'check_basic_struct:BadInputType');
...
%Test 2: throw out gen_opts.speed_sectors inputs of length not two

	gopts = make_basic_struct('gen_opts');
	gopts.speed_sectors = {3,3,4};
	
	verifyError(testCase,@()check_basic_struct(gopts,'gen_opts'),'check_basic_struct:BadInputSize');

end


function test_check_basic_struct_3(testCase)
%throw out gen_opts.speed_sectors inputs of two vectors in cell

	gopts = make_basic_struct('gen_opts');
	gopts.speed_sectors = {[3:4],[1:6]};
	
	verifyError(testCase,@()check_basic_struct(gopts,'gen_opts'),'check_basic_struct:BadInputSizeOrValue');

end


function test_check_basic_struct_4(testCase)
%throw out gen_opts.speed_sectors inputs with much quantiles asked for than exist
...
%throw out gen_opts.speed_sectors with quantiles asked for that dont exist
...
%check good one passes
	...
	check_basic_struct(gopts,'gen_opts');
...
%throw out gen_opts.speed_sectors with two inouts but not both double
