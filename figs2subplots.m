...
for k = 1:length(handles)
    ...
    elseif strcmp(get(handles(k),'Type'),'figure');
		colmaps(k).cmap = num2cell(get(handles(k),'Colormap')); %SB, have to save as cell as funny dim.
...
for k = 1:min(Ns,Na)
	...
	set(da(k),'XTick',[]); % SB to remove 0 - 1 on axis
	set(da(k),'YTick',[]); % SB
	...
	% set colormap SB
	cmap = colormap(cell2mat(colmaps(k).cmap));
	colormap(na,cmap);
	colorbar('east');
	
	
    % Produce legend if it exists in original axes
	if(~isnan(hLegParAxes)) %SB
